...
export default function StoreInformation(props) {
  ...
  //空かどうかの判定をする（テーブル用）
  const EmptyStringTable = (word, style, lavel, tag = "p") => {
    if (word !== "") {
      return (
        <tr>
          <th>{lavel}</th>
          <td style={style}>
            {tag === "a" ? (
              <p>
                <a href={"tel:+81-" + word.substr(1, 11)}>{word}</a>
              </p>
            ) : (
              <p>{word}</p>
            )}
  ...
  const Data = info => (
    <>
      <div className="kage-info" onClick={ClickEvent} />
        ...
        <StoreinBody mode={props.display_mode} state={props.state}>
          ...
          <StoreTable>
            <tbody>
              ...
              {EmptyStringTable(info.tel, {}, "電話番号", "a")}
              <tr>
                <th>地図</th>
                <td>
                  <Map />
